...
import com.tale.uder.ui.developer_settings.DeveloperSettingsSpinnerAdapter.SelectionOption;
...
@RunWith(UderRobolectricUnitTestRunner.class)
public class DeveloperSettingsSpinnerAdapterTest {
  ...
  @SuppressWarnings("NullableProblems") // Initialized in @Before.
  @NonNull private DeveloperSettingsSpinnerAdapter<SelectionOption> adapter;
  ...
  @Test public void setSelectableOptions_shouldReturnSameAdapter() {
    assertThat(adapter.setSelectionOptions(emptyList())).isSameAs(adapter);
  }

  @Test public void setSelectableOptions_shouldNotifyObservers() {
  ...
  @Test public void getCount_shouldReturn0ByDefault() {
    assertThat(adapter.getCount()).isEqualTo(0);
  }

  @Test public void getCount_shouldReturn0FromEmptyList() {
    adapter.setSelectionOptions(emptyList());
    assertThat(adapter.getCount()).isEqualTo(0);
  }

  @Test public void getCount_shouldReturnCountFromList() {
  ...
  @Test public void getItem_shouldReturnItemsFromList() {
    List<SelectionOption> selectionOptions = new ArrayList<>();
    selectionOptions.add(mock(SelectionOption.class));
    selectionOptions.add(mock(SelectionOption.class));
    selectionOptions.add(mock(SelectionOption.class));
  ...
  @Test public void getItemId_shouldReturnPositions() {
    List<SelectionOption> selectionOptions =
        asList(mock(SelectionOption.class), mock(SelectionOption.class),
  ...
  public void getView_shouldBindDataAndReturnViewWithoutConvertView() {
    ...
    for (int position = 0; position < selectionOptions.size(); position++) {
      ...
      when(layoutInflater.inflate(R.layout.list_developer_settings_spinner_item, container,
          false)).thenReturn(view);

      TextView titleTextView = mock(TextView.class);
      when(view.findViewById(R.id.list_developer_settings_spinner_item_title_text_view)).thenReturn(
          titleTextView);
  ...
  @Test public void getView_shouldBindDataAndReturnViewWithConvertView() {
    ...
    for (int position = 0; position < selectionOptions.size(); position++) {
      ...
      DeveloperSettingsSpinnerAdapter.ViewHolder viewHolder =
          mock(DeveloperSettingsSpinnerAdapter.ViewHolder.class);
  ...
  public void getDropDownView_shouldBindDataAndReturnViewWithoutConvertView() {
    ...
    for (int position = 0; position < selectionOptions.size(); position++) {
      ...
      View view = mock(View.class);
      when(
          layoutInflater.inflate(R.layout.list_developer_settings_spinner_drop_down_item, container,
              false)).thenReturn(view);
  ...
  @Test public void getDropDownView_shouldBindDataAndReturnViewWithConvertView() {
    List<SelectionOption> selectionOptions =
        asList(mock(SelectionOption.class), mock(SelectionOption.class),
            mock(SelectionOption.class));
